...
for contigfa in *contig*.fasta; do
echo "Running diamond for " $contigfa 
#Adjust number of threads with the p value
diamond blastx -d /db/diamond2/ViralRefSeqProtein -p 8 -q $contigfa -o $contigfa\_viruses.daa -k 1
done

for daa in *contig*.daa; do 
diamond view -a $daa -o $daa.m8 
done

for kt in *contig*.m8; do
